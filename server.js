...
async function initDB () {
  ...
  try {
    // Each CREATE TABLE runs in its own query so PostgreSQL can resolve
    // foreign keys one table at a time — avoids FK constraint errors.
    ...
    // Patch existing orders table — safe to run even if columns already exist
    await client.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS whatsapp       TEXT`);
    await client.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS selected_color TEXT`);
    await client.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS product_name   TEXT`);
    await client.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS quantity       INT DEFAULT 1`);
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    // derive product_name and total quantity from products_json if not supplied directly
    const _pjson = typeof b.products_json === 'string' ? b.products_json : JSON.stringify(b.products_json || '[]');
    const _items  = (() => { try { return JSON.parse(_pjson); } catch(_) { return []; } })();
    const _first  = Array.isArray(_items) && _items.length ? _items[0] : {};
    const derivedProductName = b.product_name || _first.name || null;
    const derivedQty = b.quantity != null
      ? parseInt(b.quantity)
      : (Array.isArray(_items) ? _items.reduce((s, i) => s + (parseInt(i.qty || i.quantity) || 1), 0) : 1);
    const derivedColor = b.selected_color || _first.color || _first.selected_color || null;

    const orderNum = genNum('ZMF');
    const { rows } = await db(`
      INSERT INTO orders (order_number, user_id, customer_name, customer_phone, customer_email,
        customer_city, customer_addr1, customer_addr2, whatsapp,
        selected_color, product_name, quantity,
        remark, products_json,
    ...
    `, [
      ...
      b.customer_city||null, b.customer_addr1||null, b.customer_addr2||null,
      b.whatsapp||null,
      derivedColor, derivedProductName, derivedQty,
      b.remark||null, _pjson,
