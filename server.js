...
async function initDB() {
  try {
    ...
    await dbQuery(`ALTER TABLE products ADD COLUMN IF NOT EXISTS deal_ends_at TIMESTAMPTZ;`).catch(() => {});
      ...
      const result = await dbQuery(`
        INSERT INTO products
          (name, description, category_slug, price, previous_price, save_amount,
           ...
           specifications, how_to_use, available_colors, deal_ends_at)
    ...
    try {
      ...
      const result = await dbQuery(`
        UPDATE products SET
          ...
          specifications=$17, how_to_use=$18, available_colors=$19, deal_ends_at=$20, updated_at=NOW()
      ...
      `, [
        ...
        b.deal_ends_at !== undefined ? (b.deal_ends_at || null) : old.deal_ends_at,
        req.params.id,
      ]);
...
// ═══════════════════════════════════════════════════════════════════════════════
// PUBLIC — Recent Purchases (social proof popup, safe fields only)
// ═══════════════════════════════════════════════════════════════════════════════
  ...
  try {
    const result = await dbQuery(`
      SELECT
        ...
        product_name,
        product_image,
        TO_CHAR(created_at::timestamptz AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"') AS ordered_at
      FROM orders
    ...
    return sendError(res, 500, 'Failed to fetch recent purchases.', err.message);
  }
});
